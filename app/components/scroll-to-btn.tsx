    ...
    return (
        <div className="fixed bottom-0 left-0 w-screen h-12 flex justify-evenly">
            <Link href={'#'} className={'mx-0.3 animate-pulse'} onClick={() => scrollTo(activeSectionIdx)}>
                {activeSectionIdx > 0
                    ? (<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className={`font-semibold w-8 h-8 ${activeSectionIdx === 4  ? 'text-white hover:text-primary' : 'text-primary hover:text-seconday'}`}>
                    ...
                    : (<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={`font-semibold ${activeSectionIdx === 4  ? 'text-white hover:text-primary' : 'text-primary hover:text-seconday'} w-8 h-8`}>
